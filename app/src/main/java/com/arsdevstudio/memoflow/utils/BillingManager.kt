...
class BillingManager(private val context: Context, private val billingPrefs: BillingPrefs) {
    ...
    private fun queryProducts() {
        val productList = listOf(
            QueryProductDetailsParams.Product.newBuilder()
                .setProductId("premiumlifetime")
                .setProductType(BillingClient.ProductType.INAPP)
                .build(),
            QueryProductDetailsParams.Product.newBuilder()
                .setProductId("donationcoffee")
                .setProductType(BillingClient.ProductType.INAPP)
                .build(),
            QueryProductDetailsParams.Product.newBuilder()
                .setProductId("donationsnack")
                .setProductType(BillingClient.ProductType.INAPP)
                .build(),
            QueryProductDetailsParams.Product.newBuilder()
                .setProductId("donationmeal")
                .setProductType(BillingClient.ProductType.INAPP)
                .build()
        )
        ...
        billingClient.queryPurchasesAsync(params) { billingResult, purchases ->
            if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                ...
                for (purchase in purchases) {
                    if (purchase.products.contains("premiumlifetime") && purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
                        hasPremium = true
                        handlePurchase(purchase)
                    } else if (purchase.products.any { it.startsWith("donation") }) {
                        handlePurchase(purchase)
                    }
                }
    ...
    private fun handlePurchase(purchase: Purchase) {
        if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
            ...
            if (productId == "premiumlifetime" && !purchase.isAcknowledged) {
            ...
            if (productId.startsWith("donation")) {
